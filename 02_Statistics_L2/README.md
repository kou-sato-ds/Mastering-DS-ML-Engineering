...
## 🛠️ 実装プロジェクト3：推測統計（信頼区間による意思決定支援）

### 1. 概要
単なる平均値（点推定）だけでなく、t分布を用いた **95%信頼区間** を算出するロジックを実装しました。

### 2. ビジネス上の価値（Why this matters）
データには必ず「ブレ」が存在します。
- **意思決定の精度向上**: 「売上平均は100万円です」と断言するのではなく、「95%の確率で95万〜105万円の範囲に収まります」と提示することで、ワーストケースを想定したリスク管理が可能になります。
- **少標本への対応**: データ数が少ない場合でも、t分布を用いることで過小評価を防ぎ、堅実な推論を提供します。

### 3. 解析結果
実装したロジックにより、サンプルデータから以下の区間を導出しました。
- **推定値**: 101.00
- **95%信頼区間**: 97.97 〜 104.03
---
